document.addEventListener('DOMContentLoaded', () => {
    ...
    function createTodoItem(text, isCompleted = false) {
        ...
        const deleteButton = document.createElement('buon');
        deleteButton.innerHTML = '';
        deleteButton.classList.add('delete');
        deleteButton.addEventListener('click', () => {
            const confirmed = confirm('Kok Bisa Tau Ada Tombol Ini,\nIni Tombol Hapus');
            if (confirmed) {
    ...
    function saveEdit() {
        ...
        if (confirmed && currentEditSpan) {
            currentEditSpan.textContent = editInput.value.trim() || 'Unname Teks';
            closePopup();
            saveTodos();
        }
    }
